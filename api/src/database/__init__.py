...
def new_task(user_id: str, task_name: str, initial_df: DataFrame, initial_df_frontend: DataFrame | None = None):
    """ Create a new task for a new initial_df DataFrame or retrieve an existing task with task_id """
    return Task(user_id=user_id, task_name=task_name, initial_df=initial_df, initial_df_frontend=initial_df_frontend)
